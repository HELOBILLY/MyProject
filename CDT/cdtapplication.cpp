...
bool CDTApplication::initDatabase()
{
    ...
    ///  Create segmentation layer table(id text,name text,shapefilePath text,color blob,imageID text).
...
//                     "opacity double,"
    ...
    /// Create merge layer table(id text,name text, shpfilePath text...
    ret = query.exec("CREATE TABLE mergelayer"
                     "(id text NOT NULL, "
                     "name text NOT NULL,"
                     "shapefilePath text NOT NULL,"
                     "color blob,"
    ...
    if(ret == false)
    {
        QMessageBox::critical(NULL,tr("Error"),tr("create table mergelayer failed!\nerror:")+query.lastError().text());
        Log4Qt::Logger::rootLogger()->error("create table mergelayer failed!\nerror msg:%1",query.lastError().text());
        return false;
    }
    /// Create overlay layer table(id text,name text, shpfilePath text...
    ret = query.exec("CREATE TABLE overlayer"
                     "(id text NOT NULL, "
                     "name text NOT NULL,"
                     "shapefilePath text NOT NULL,"
                     "color blob,"
    ...
    if(ret == false)
    {
        QMessageBox::critical(NULL,tr("Error"),tr("create table overlayer failed!\nerror:")+query.lastError().text());
        Log4Qt::Logger::rootLogger()->error("create table overlayer failed!\nerror msg:%1",query.lastError().text());
        return false;
    }
    ...
    ///  Create detect layer table (id text,name text,shpfilePath text,imageID text).
    ret = query.exec("CREATE TABLE detectionlayer"
                     "(id text NOT NULL, "
                     "name text NOT NULL,"
                     "shapefilePath text NOT NULL,"
                     "dbUrl blob,"
    ...
    if (ret == false)
    {
        QMessageBox::critical(NULL,tr("Error"),tr("create table detection layer failed!\nerror:")+query.lastError().text());
        Log4Qt::Logger::rootLogger()->error("create table detection layer failed!\nerror msg:%1",query.lastError().text());
        return false;
    }
