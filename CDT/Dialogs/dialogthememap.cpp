...
DialogThemeMap::DialogThemeMap(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::DialogThemeMap)
{
    ui->setupUi(this);
    ui->comboBox_StandTemplage->addItem(tr("A3_Landscape_Blue"));
    ui->comboBox_StandTemplage->addItem(tr("A3_Landscape_Orange"));
    ui->comboBox_StandTemplage->addItem(tr("A3_Portrait_Blue"));
    ui->comboBox_StandTemplage->addItem(tr("A3_Portrait_Orange"));
    ui->comboBox_StandTemplage->addItem(tr("A4_Landscape_Blue"));
    ui->comboBox_StandTemplage->addItem(tr("A4_Landscape_Orange"));
    ui->comboBox_StandTemplage->addItem(tr("A4_Portrait_Blue"));
    ui->comboBox_StandTemplage->addItem(tr("A4_Portrait_Orange"));
}

DialogThemeMap::~DialogThemeMap()
{
    delete ui;
}

void DialogThemeMap::on_radioButton_Current_clicked()
{

}
//Browse new customized map template
void DialogThemeMap::on_pushButton_clicked()
{
...
void DialogThemeMap::on_pushButton_OutPut_PDF_clicked()
{
...
//        QMessageBox::information(NULL, "LayerName", layername, QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
        ...
        foreach (DetectLayer *dlayer, pDetectionLayers) {
            ...
            QByteArray ba = dlayername.toLatin1();
            poDataset = (GDALDataset*) GDALOpen( ba.data(),GA_ReadOnly );
            qDebug()<< poDataset->GetProjectionRef();*/
...
void DialogThemeMap::on_pushButton_Output_Img_clicked()
{
