    ...
    for (int k=0;k<3;++k)
    {
        ...
        if(dfMin>=0&&dfMax<=255)
        {
            ...
            //        maxBandVal_texture[k]=dfMean+2*dfStdDev;
                    ...
                    int lowDes = (int) (histRatioL * nXSize*nYSize);
                    int highDes = (int)(histRatioH * nXSize*nYSize);
                    int lh[2] = {0, 255};
                    int h,sum;
...
CDTAutoGrabcutMapTool::CDTAutoGrabcutMapTool(QgsMapCanvas *canvas) :
    QgsMapTool(canvas),
    mRubberBand(NULL),
    vectorLayer(NULL)
{
...
CDTAutoGrabcutMapTool::~CDTAutoGrabcutMapTool()
{
    delete mRubberBand;
}

void CDTAutoGrabcutMapTool::canvasMoveEvent(QMouseEvent *e)
{
    if ( mRubberBand == NULL )
    {
        return;
...
void CDTAutoGrabcutMapTool::canvasPressEvent(QMouseEvent *e)
{
    if ( mRubberBand == NULL )
    {
